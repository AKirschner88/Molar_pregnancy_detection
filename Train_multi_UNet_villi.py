...
class Dataset(object): 
    ...
    def __getitem__(self, index):
        with tables.open_file(self.fname, 'r') as db:
            ...
            mask = db.root.mask[index]  # original labels: 0/1/2
    
        # augment first on original mask (keeps labels stable)
        if self.transforms:
        ...
        if n_classes == 2:
            orig = mask.astype(np.int16, copy=True)
            mask = np.zeros_like(orig, dtype=np.int16)
            mask[orig == 2] = 1
            mask[orig == 1] = -100
    
        # edge weighting (avoid ignored pixels)
        if self.edge_weight:
            pos = (mask == 1)
            valid = (mask != -100)
            eweight = (skimage.morphology.binary_dilation(pos, skimage.morphology.square(1)) &
                       (~pos) & valid).astype(np.uint8)
        else:
            eweight = np.zeros(mask.shape, dtype=np.uint8)
...
dataname = 'AKV10_class2only_data_villi_multi' 
...
# unet params
n_classes = 2  # unlike classical unets, we have 3 classes
...
if n_classes == 2:
    # binary setup: 0=background, 1=class2; class1 is ignored (-100)
    class_weight = None
    print("class weight: None (ignore_index used for class1)")
else:
    class_weight = dataset["train"].numpixels[1, :3]  # original 3 classes
    ...
    for phase in phases:
        ...
        for _, (X, y, y_weight) in enumerate(dataLoader[phase]):
            ...
            with torch.set_grad_enabled(phase == 'train'):
                ...
                x_pred = model(X)
                y_pred_argmax = torch.argmax(x_pred, dim=1)
                ...
                if phase in validation_phases:
                    ...
                    predflat = y_pred_argmax.flatten()
                    targetflat = y.flatten()
                    
                    valid = (targetflat != -100)
                    predflat = predflat[valid]
                    targetflat = targetflat[valid]
...
state = torch.load(
    r"E:\Digital_Pathology\Project\Aron_to_share\Models\AKV7_9_E2_data_villi_multi_best_model_multi_UNet.pth",
    map_location=device
)
