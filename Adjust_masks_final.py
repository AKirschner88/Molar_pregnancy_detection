...
from scipy.ndimage import binary_fill_holes, binary_propagation, distance_transform_edt
...
# Output split:
OUT_GEOJSON_ADJUSTED = r"E:\Digital_Pathology\Project\Aron_to_share\Annotations\Adjusted_annotations\adjustedagain\DP00002001_adjusted.geojson"
OUT_GEOJSON_FAILED   = r"E:\Digital_Pathology\Project\Aron_to_share\Annotations\Adjusted_annotations\adjustedagain\DP00002001_failed.geojson"
...
OD_BAND_PX = 25
OD_SMOOTH_BARRIER_R = 1
OD_SMOOTH_BG_EDGE_R = 0

# --- Bridge/thickness filters ---
BRIDGE_R = 0
THICK_MIN = 0
...
# -----------------------------
# SciPy-safe structuring element
# -----------------------------
def se_disk(r: int):
    r = int(r)
    if r <= 0:
        return np.ones((1, 1), dtype=bool)
    return disk(r).astype(bool)
...
def rasterize_geom_to_mask(geom, x0, y0, wL, hL, ds):
    """
    Rasterize a level-0 shapely geometry into a bool mask in ROI coordinates
    at pyramid level with downsample factor ds.

    Inputs:
      geom : shapely Polygon/MultiPolygon in level-0 coords
      x0,y0: ROI top-left in level-0 coords (same as read_region location)
      wL,hL: ROI width/height in LEVEL pixels (size passed to read_region at that level)
      ds   : float downsample for that level (slide.level_downsamples[level])
    ...
    def to_roi_level_xy(coords_xy):
        # coords_xy: Nx2 in level-0 coords -> ROI local in level pixels
    ...
    if isinstance(geom, Polygon):
        ...
        # try to fix invalid/other geom types
        g2 = geom.buffer(0)
...
def background_mask_white(rgb_u8, Lmin, cmax):
    ...
    return (L >= float(Lmin)) & (chroma <= float(cmax))
...
def bg_touching_annotation_boundary_odbarrier(
    ...
    if vals.size == 0:
        ...
        T = float(np.quantile(vals, float(q_edge)))
        edge_barrier = (grad >= T) & ann_mask

    # smooth barrier (SciPy-safe disks)
    if smooth_barrier_r and smooth_barrier_r > 0:
        se = se_disk(smooth_barrier_r)
...
def mask_to_best_polygon_level(mask, x0, y0, ds):
    """
    Convert a boolean mask at pyramid level (downsample ds) into a Polygon in LEVEL-0 coords.
    x0,y0 are ROI origin in LEVEL-0 coords (same as read_region location).
    """
    ...
    for c in contours:
        # c: (row, col) in level pixels -> level-0 coords
        xs0 = x0 + c[:, 1] * ds
        ys0 = y0 + c[:, 0] * ds
        xy0 = np.column_stack([xs0, ys0])
    ...
    # IMPORTANT: tolerances are in level-0 pixels now (good)
    ...
    if SMOOTH_BUF and SMOOTH_BUF > 0:
...
def repair_mask_per_component(new_mask, ann_mask, r_close=0):
    ...
    if r_close and r_close > 0:
        se = se_disk(r_close)
        comp = ndi.binary_dilation(comp, structure=se)
        comp = ndi.binary_erosion(comp, structure=se)
...
def adjust_annotation_geom(slide, geom):
    ...
    # ROI size in level pixels
    wL = max(1, int(round(w / ds)))
    hL = max(1, int(round(h / ds)))
    
    # read ROI at level
    rgb = np.array(slide.read_region((x0, y0), LVL, (wL, hL)).convert("RGB"), dtype=np.uint8)
    
    # rasterize to the SAME level grid (IMPORTANT)
    ann_mask = rasterize_geom_to_mask(geom, x0, y0, wL, hL, ds)
    ...
    # --- bg_edge ---
    if USE_OD_BARRIER:
        bg_edge, _, _ = bg_touching_annotation_boundary_odbarrier(
            rgb, bg, ann_mask,
            q_edge=OD_Q_EDGE,
    ...
    if THICK_MIN and THICK_MIN > 0:
        ...
        se = se_disk(THICK_MIN)
        bg_edge = ndi.binary_dilation(core, structure=se) & bg_edge
    ...
    if SMOOTH_RADIUS and SMOOTH_RADIUS > 0:
        new_mask = binary_closing(new_mask, footprint=disk(int(SMOOTH_RADIUS)))
    ...
    poly = mask_to_best_polygon_level(new_mask, x0, y0, ds)
    if poly is None:
        return geom, False
...
# -----------------------------
# DEBUG ONE FEATURE (full steps)
# -----------------------------
def debug_one_feature(idx, lvl=3):
    slide = openslide.OpenSlide(SLIDE_TIFF)
    ds = float(slide.level_downsamples[lvl])
    print("level:", lvl, "downsample:", ds)
    ...
    # --- ROI in level-0 coords ---
    minx, miny, maxx, maxy = geom.bounds
    x0 = int(math.floor(minx)) - PAD_PX
    y0 = int(math.floor(miny)) - PAD_PX
    w0 = int(math.ceil(maxx - minx)) + 2 * PAD_PX
    h0 = int(math.ceil(maxy - miny)) + 2 * PAD_PX

    W, H = slide.dimensions
    x0 = max(0, x0); y0 = max(0, y0)
    w0 = min(w0, W - x0); h0 = min(h0, H - y0)
    ...
    # --- rasterize annotation into SAME level grid ---
    ann_mask = rasterize_geom_to_mask(geom, x0, y0, wL, hL, ds)

    if not ann_mask.any():
        print("EMPTY ann_mask at idx", idx)
        return
    ...
    if BRIDGE_R and BRIDGE_R > 0:
        se = se_disk(BRIDGE_R)
        seed = ndi.binary_erosion(bg_edge, structure=se)
        bg_edge = reconstruction(seed.astype(np.uint8), bg_edge.astype(np.uint8), method="dilation").astype(bool)
    ...
    if SMOOTH_RADIUS and SMOOTH_RADIUS > 0:
        new_mask = binary_closing(new_mask, footprint=se_disk(SMOOTH_RADIUS))
        new_mask = remove_small_holes(new_mask, MIN_HOLE_AREA)
        new_mask = binary_fill_holes(new_mask)
    ...
    fail_reasons = []
    if new < MIN_KEEP_RATIO * orig:
        fail_reasons.append(f"MIN_KEEP_RATIO: new={new} < {MIN_KEEP_RATIO:.2f}*orig={orig}")
    if orig < SMALL_ORIG_PX and removed > MAX_REMOVE_SMALL:
        fail_reasons.append(f"MAX_REMOVE_SMALL: removed={removed} > {MAX_REMOVE_SMALL} (orig={orig})")

    # polygon back in LEVEL-0 coords (IMPORTANT)
    poly = mask_to_best_polygon_level(new_mask, x0, y0, ds)
    if poly is None:
        fail_reasons.append("mask_to_best_polygon_level failed")

    passed = (len(fail_reasons) == 0)

    print("\n==============================")
    print("idx:", idx, "| level:", lvl, "| ds:", ds)
    print("ROI level0 w,h:", w0, h0, " | level w,h:", wL, hL)
    ...
    # --- overlay boundaries on LEVEL image (for visualization only) ---
    overlay = rgb.copy()
    ...
    LINE_W_L0 = 10                       # desired thickness in level-0 pixels
    LINE_W_L  = max(1, int(round(LINE_W_L0 / ds)))  # convert to level pixels
    
    b_old = ndi.binary_dilation(b_old, structure=np.ones((3,3), np.uint8), iterations=LINE_W_L)
    b_new = ndi.binary_dilation(b_new, structure=np.ones((3,3), np.uint8), iterations=LINE_W_L)
    ...
    ax[0].imshow(rgb); ax[0].set_title(f"RGB (level {lvl})"); ax[0].axis("off")
    ax[1].imshow(ann_mask, cmap="gray"); ax[1].set_title("ann_mask (level grid)"); ax[1].axis("off")
    ax[2].imshow(bg, cmap="gray"); ax[2].set_title(f"bg LAB (L>={BG_L_MIN}, C<={BG_CHROMA_MAX})"); ax[2].axis("off")
    ax[3].imshow(edge_barrier, cmap="gray"); ax[3].set_title("edge_barrier (OD)"); ax[3].axis("off")

    ax[4].imshow(bg_edge_before_filters, cmap="gray"); ax[4].set_title("bg_edge BEFORE filters"); ax[4].axis("off")
    ax[5].imshow(bg_edge_after_filters, cmap="gray"); ax[5].set_title("bg_edge AFTER filters"); ax[5].axis("off")

    ax[6].imshow(new_mask_raw, cmap="gray"); ax[6].set_title("new_mask_raw"); ax[6].axis("off")
    ax[7].imshow(new_mask, cmap="gray"); ax[7].set_title("new_mask FINAL"); ax[7].axis("off")
    ...
    ax[10].hist(L[ann_mask].ravel(), bins=50); ax[10].set_title("L inside ann (level grid)")
    ax[11].hist(chroma[ann_mask].ravel(), bins=50); ax[11].set_title("Chroma inside ann (level grid)")

    plt.tight_layout()
    plt.show()

    # return poly if you want to inspect it
    return poly, passed
...
def main():
    slide = openslide.OpenSlide(SLIDE_TIFF)
    ds = slide.level_downsamples[3]
    print("level3 downsample:", ds)
    ...
    for feat in tqdm(features, total=len(features), desc="Adjusting annotations"):
        ...
        if passed:
            ...
            feat_bad["geometry"] = mapping(geom)
...
test_idxs = [115,125,135,145,156,166,177,188,199]

for i in test_idxs:
    # you can tweak globals above this call
    debug_one_feature(i, lvl=3)
    print("\n==============================")
    print("Testing idx:", i)
