...
READ_LEVEL = 4  # <-- run everything at OpenSlide level 3
...
def main():
    ...
    OUTER_BAND_PX = scale_px_int(OUTER_BAND_PX_L0, ds, min_val=1)
    INNER_MIN_DIST_PX = scale_px_int(INNER_MIN_DIST_PX_L0, ds, min_val=1)

    OPEN_RADIUS = scale_px_int(OPEN_RADIUS_L0, ds, min_val=0)  # allow 0
    CLOSE_RADIUS = scale_px_int(CLOSE_RADIUS_L0, ds, min_val=0)  # allow 0

    # area scales with ds^2
    ...
    for FEATURE_INDEX in rand_indices:
        ...
        H_chan = rgb2hed(rgb_f)[..., 0]
        H_smooth = gaussian(H_chan, sigma=SIGMA_H, preserve_range=True)
    
        # Distance inside mask (in READ_LEVEL pixels)
        dist_in = distance_transform_edt(ann_mask)
    
        # Inner eligible region
        inner_region = ann_mask
        if inner_region.sum() < 200:
            fallback = max(OUTER_BAND_PX + 2, int(dist_in.max() * 0.3))
            inner_region = ann_mask & (dist_in >= fallback)
    
        vals_inner = H_smooth[inner_region]
        if vals_inner.size == 0:
            print("  -> skipped (no pixels in inner_region)")
            continue
    
        thr = np.quantile(vals_inner, INNER_QUANTILE)
    
        # Raw seed mask
        seed2_raw = inner_region & (H_smooth <= thr)
        ...
        # Convert seed2 to one outer boundary and fill -> class2
        class2 = filled_from_outer_contour(seed2, ann_mask.shape, smooth_win=SMOOTH_WIN) & ann_mask
    
        # Plot (one figure per random feature)
        fig, ax = plt.subplots(1, 3, figsize=(18, 6))
        ax[0].imshow(rgb); ax[0].set_title(f"RGB (ROI) - level {READ_LEVEL}"); ax[0].axis("off")
        ax[1].imshow(rgb); ax[1].imshow(seed2_raw, alpha=0.35); ax[1].set_title(f"Seed2 raw (idx={FEATURE_INDEX})"); ax[1].axis("off")
        ax[2].imshow(rgb)
        ...
        ax[2].set_title("Class2 = filled outer contour of seed2")
        ax[2].axis("off")
        plt.tight_layout()
        plt.show()
